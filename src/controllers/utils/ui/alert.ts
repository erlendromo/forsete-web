...
export function createSuccessAlert(message: string): string {
  return `<div class="flex items-center p-4 mb-4 text-green-800 border-t-4 border-green-400 bg-green-100 dark:text-green-400 dark:bg-gray-800 dark:border-green-800"
      role="alert">
      <svg class="shrink-0 w-4 h-4" aria-hidden="true" fill="currentColor" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20">
        <path d="M16.707 5.293a1 1 0 ...Z"/>
      </svg>
      <div class="ms-3 text-sm font-medium">
        ${message}
      </div>
      <button type="button" id="close-alert-button" class="ms-auto -mx-1.5 -my-1.5 bg-green-100 text-green-500 rounded-lg focus:ring-2 focus:ring-green-500 p-1.5 hover:bg-green-200 inline-flex items-center justify-center h-8 w-8
      dark:bg-gray-800 dark:text-green-400 dark:hover:bg-gray-700"aria-label="Close">
        <span class="sr-only">Dismiss</span>
